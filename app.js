...
app.get(
  ...
  async (request, response) => {
    ...
    select count(reply_id) ad replies from reply where tweet_id = ${tweetId};`);
    response.send({ tweet, likes, replies, dateTime: date_time });
  }
);

// get all likes of a tweet with id if the user follows the tweeter
...
app.get(
  ...
  async (request, response) => {
    ...
    const replies = await db.all(`
        ...
        reply natural join user where tweet_id = ${tweetId};
        `);
...
app.get("/user/tweets/", authenticateToken, async (request, response) => {
  const myTweets = await db.all(`
    select
    tweet.tweet,
    count(distinct like.like_id) ad likes,
